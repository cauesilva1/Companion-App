-- Sono humano: dorme após horário de dormir; só acorda com app aberto depois das 6h.
-- Abrir o app antes das 6h mantém sleep (tom sonolento no cliente).
...
BEGIN
  ...
  -- Modo diurno (se estivesse acordado)
  IF on_home IS FALSE
  ...
  -- Janela de dormir: 23h–5h59 (hora local)
  in_bed_window := (hour_local >= 23 OR hour_local < 6);
  -- Acorda só com app em foreground DEPOIS das 6h e antes da hora de dormir
  ...
  IF old_mode = 'sleep'::"LifeMode" THEN
    IF can_wake THEN
      new_mode := day_mode;
    ELSE
      -- Continua dormindo (inclui abrir o app antes das 6h → sonolento)
  ...
  IF new_mode = 'sleep'::"LifeMode" AND (
    ...
    morning := format(
      'Bom dia — %s dormiu de verdade. Energia %s%%, afeto %s%%.',
      c.name,
      c.energy::text,
      c.affection::text
    );
  ...
  IF old_mode = 'sleep'::"LifeMode" AND new_mode <> 'sleep'::"LifeMode" THEN
    IF morning IS NULL OR c."morningThoughtDayKey" IS DISTINCT FROM day_key THEN
      morning := format(
        'Bom dia! %s acordou quando você abriu o app (modo %s).',
        c.name,
        new_mode::text
      );
      seeded_morning := true;
  ...
  IF c."mediaHint" IS DISTINCT FROM prev_media
     ...
     AND length(c."mediaHint") > 0
     AND c."lifeMode" <> 'sleep'::"LifeMode" THEN
